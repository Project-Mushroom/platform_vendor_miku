...
ifeq ($(TARGET_MIKU_BUILD_VARIANT),OFFICIAL)
    ...
    PROD_VERSION += MikuUI-$(PLATFORM_MIKU_VERSION)-$(TARGET_DEVICE)-$(shell date +%y%m%d%H%M)-$(MIKU_TYPE)OFFICIAL
else ifeq ($(TARGET_MIKU_BUILD_VARIANT),COMMUNITY)
    PLATFORM_MIKU_VERSION := $(PLATFORM_MIKU_RELEASE)
    PROD_VERSION += MikuUI-$(PLATFORM_MIKU_VERSION)-$(TARGET_DEVICE)-$(shell date +%y%m%d%H%M)-$(MIKU_TYPE)COMMUNITY
else
    PLATFORM_MIKU_VERSION := $(PLATFORM_MIKU_RELEASE)
    TARGET_MIKU_BUILD_VARIANT := UNOFFICIAL
    PROD_VERSION += MikuUI-$(PLATFORM_MIKU_VERSION)-$(TARGET_DEVICE)-$(shell date +%y%m%d%H%M)-$(MIKU_TYPE)UNOFFICIAL
endif
